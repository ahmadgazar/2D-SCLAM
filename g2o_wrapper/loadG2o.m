...
function [landmarks, poses, transitions, observations, odometry] = loadG2o(filepath)
	...
	while true
		...
		switch(elements{1})
			...
			case JOINT_STATE
			  	 odometry(end+1) = extractOdometry(elements);
			   	 i_joint_state = i_joint_state + 1;
			otherwise
				disp('Error in reading first element');
		end
	end

  printf('[G2oWrapper] loading file...\n#landmarks: %d \n#encoders measurements: %d \n', i_vert_xy, i_joint_state);
  printf('#transitions: %d \n#observation: %d \n',i_edge_se2, i_edge_se2_xy);
...
function out = extractOdometry(elements)
  id = str2double(elements{2});
  l_encoder = str2double(elements{3});
  r_encoder = str2double(elements{4});
  out = odometry(id, l_encoder, r_encoder);
end
